...
def update_log_path() -> Path:
    if os.name == "nt":
        ...
        base = Path.home()
    return base / "SyncRoom" / "update.log"
...
class MainWindow(QMainWindow):
    ...
    def launch_update_installer_and_exit(self, installer_path: Path) -> None:
        ...
        helper_path = app_path.with_name("SyncRoomUpdater.exe")
        if not helper_path.exists():
            append_update_log(f"Update helper missing at {helper_path}")
            self.show_error("SyncRoomUpdater.exe was not found next to the app.")
            return
        creationflags = 0
        if os.name == "nt":
            creationflags = subprocess.CREATE_NEW_PROCESS_GROUP | subprocess.DETACHED_PROCESS
        append_update_log(
            f"Launching updater helper helper={helper_path} installer={installer_path} app={app_path}"
        )
        process = subprocess.Popen(
            [str(helper_path), str(installer_path), str(app_path), str(os.getpid())],
        ...
        if exit_code is not None:
            append_update_log(f"Updater helper exited early with code {exit_code}")
            self.show_error(
                "SyncRoom could not start the updater helper. Install the new version manually this time."
            )
            return
        append_update_log("Updater helper started successfully")
...
def report_startup_crash(exc: BaseException) -> None:
    ...
    path = Path.home() / "syncroom-startup-crash.log"
    try:
