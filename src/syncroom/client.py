...
class MainWindow(QMainWindow):
    ...
    def launch_update_installer_and_exit(self, installer_path: Path) -> None:
        ...
        helper_dir = Path(tempfile.mkdtemp(prefix="syncroom-updater-"))
        helper_path = helper_dir / "SyncRoomUpdater.exe"
        shutil.copy2(app_path, helper_path)
        creationflags = 0
        if os.name == "nt":
...
def main() -> None:
    if os.name == "nt" and len(sys.argv) >= 5 and sys.argv[1] == "--apply-update":
        sys.exit(run_windows_update_helper(sys.argv[2], sys.argv[3], int(sys.argv[4])))
