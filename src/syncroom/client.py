...
from syncroom.settings import default_display_name, load_settings, save_settings
...
class SyncClient(QObject):
    ...
    def connect_to_server(self, host: str, port: int, room: str, name: str) -> None:
        self.room = room
        self.name = name
        self.socket.connectToHost(host, port)
        if self.socket.waitForConnected(3000):
            self.send({"type": "join", "room": room, "name": name})
...
class StartupSetupDialog(QDialog):
    def __init__(self) -> None:
        ...
        eyebrow = QLabel("FIRST-TIME SETUP")
        eyebrow.setObjectName("eyebrow")
        title = QLabel("Installing the video player")
        title.setObjectName("title")
        title.setWordWrap(True)
        subtitle = QLabel(
            "SyncRoom is downloading and preparing mpv for Windows. The app will open automatically when setup finishes."
        )
...
class MainWindow(QMainWindow):
    ...
    def build_join_page(self) -> QWidget:
        ...
        self.host_input = QLineEdit(self.settings.get("host", "127.0.0.1"))
        ...
        grid.addWidget(self.host_input, 0, 0, 1, 2)
        grid.addWidget(self.port_input, 1, 0)
        grid.addWidget(self.room_input, 1, 1)
        grid.addWidget(self.name_input, 2, 0, 1, 2)
    ...
    def connect_to_room(self) -> None:
        ...
        self.sync_client.connect_to_server(host, port, room, name)
    ...
    def apply_pending_room_sync(self) -> None:
        ...
        try:
            ...
            drift = abs(int(local["position_ms"]) - position_ms)
            self.suppress_sync = True
            should_force_seek = seek_token > self.last_applied_seek_token
            if should_force_seek or drift > 500:
                self.player.seek_absolute(position_ms)
                self.last_applied_seek_token = max(self.last_applied_seek_token, seek_token)
            if playing:
                if not local["playing"]:
                    self.player.play()
            elif local["playing"]:
                self.player.pause()
                if drift > 250:
            ...
            if remaining_drift > (1800 if playing else 450):
    ...
    def on_slider_released(self) -> None:
        ...
        if self.current_media_url:
            try:
                status = self.player.get_status()
                if bool(status["playing"]):
                    self.position_slider.setValue(int(status["position_ms"]))
                    self.position_label.setText(self.format_ms(int(status["position_ms"])))
                    self.show_status("Pause before seeking")
                    return
                ...
                self.local_seek_override_until = time.monotonic() + 1.2
    ...
    def poll_player_state(self) -> None:
        ...
        if not status.get("running", True):
            if self.page_stack.currentIndex() == 1 and not self.closing_for_mpv_exit:
        ...
        if state_changed:
            self.sync_client.send_state(self.current_media_url, position, playing)
        self.last_known_playing = playing
    ...
    def on_room_state(self, payload: dict) -> None:
        ...
        try:
            ...
            self.suppress_sync = True
            should_force_seek = seek_token > self.last_applied_seek_token
            if should_force_seek or drift > 850:
                self.player.seek_absolute(position_ms)
                self.last_applied_seek_token = max(self.last_applied_seek_token, seek_token)
            if playing and not local["playing"]:
                self.player.play()
            elif not playing and local["playing"]:
                self.player.pause()
                if drift > 250:
                    self.player.seek_absolute(position_ms)
            self.last_known_playing = playing
            ...
            if "property unavailable" in str(exc).lower() or "mpv is not running" in str(exc).lower():
                ...
                self.pending_room_sync = payload
                if self.pending_room_sync_attempts <= 0:
                    self.pending_room_sync_attempts = 8
                QTimer.singleShot(500, self.apply_pending_room_sync)
            else:
                self.show_error(f"Could not sync mpv state: {exc}")
        finally:
            self.suppress_sync = False
    ...
    def apply_audio_preferences(self) -> bool:
        ...
        try:
            ...
            self.show_error(f"Could not apply audio preference: {exc}")
            return False
        if track:
    ...
    def refresh_audio_tracks(self, silent: bool = False) -> None:
        ...
        try:
            ...
            if not silent:
                self.show_error(f"Could not read audio tracks: {exc}")
            return
    ...
    def launch_update_installer_and_exit(self, installer_path: Path) -> None:
        ...
        script_path.write_text(
            "\n".join(
                [
                    ...
                    ")",
                    'start "" /wait "%INSTALLER%" /SP- /VERYSILENT /SUPPRESSMSGBOXES /CLOSEAPPLICATIONS',
        ...
        self.show_status("Installing update and restarting SyncRoom...")
