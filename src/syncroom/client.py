...
def configure_crash_logging() -> None:
    ...
    log_root = logs_dir()
    runtime_path = log_root / "runtime.log"
    crash_path = log_root / "crash.log"
    _FAULT_LOG_HANDLE = crash_path.open("a", encoding="utf-8")
...
class SyncClient(QObject):
    ...
    def _handle_message(self, payload: dict) -> None:
        ...
        if msg_type == "welcome":
            ...
            append_runtime_log(
                "Received room_state "
                f"media={'yes' if payload.get('media_url') else 'no'} "
                f"playing={bool(payload.get('playing'))} "
                f"position={int(payload.get('position_ms') or 0)} "
...
class MainWindow(QMainWindow):
    ...
    def set_media_url(self, media_url: str, broadcast: bool) -> None:
        ...
        append_runtime_log(f"set_media_url broadcast={broadcast} media_url={media_url}")
        try:
    ...
    def apply_server_state(self, payload: dict, settle_mode: bool = False) -> bool:
        ...
        append_runtime_log(
            "apply_server_state "
            f"settle_mode={settle_mode} local_running={local.get('running')} "
            f"local_playing={local.get('playing')} target_playing={playing} "
            f"target_position={position_ms} event_id={event_id} action={last_action}"
        )
    ...
    def apply_pending_room_sync(self) -> None:
        ...
        try:
            append_runtime_log(
                f"apply_pending_room_sync attempts_left={self.pending_room_sync_attempts} current_media={self.current_media_url}"
            )
    ...
    def poll_player_state(self) -> None:
        ...
        try:
            ...
            append_runtime_log(f"poll_player_state get_status failed: {exc}")
            return
    ...
    def on_room_state(self, payload: dict) -> None:
        ...
        if media_url != self.current_media_url:
            append_runtime_log(
                f"Late join or media switch detected current={self.current_media_url or '<none>'} incoming={media_url}"
            )
